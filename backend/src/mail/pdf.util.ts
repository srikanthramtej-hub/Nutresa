...
export function generateOrderPDF(order: any, gst?: { enabled: boolean; rate: number }): Promise<Buffer> {
  return new Promise((resolve, reject) => {
    ...
    doc.fontSize(9).font('Helvetica').fillColor('#f3e4c9')
      .text(`Order Confirmation · ${new Date(order.createdAt || Date.now()).toLocaleDateString('en-IN', { day:'numeric', month:'long', year:'numeric' })}`,
        40, 46, { align: 'right', width: doc.page.width - 80 })
    ...
    items.forEach((item: any, idx: number) => {
      const rowH = 28
      if (idx % 2 === 0) doc.rect(40, y, doc.page.width - 80, rowH).fill('#faf7f7')
    ...
    doc.fillColor(GRAY).fontSize(8).font('Helvetica')
      .text('Thank you for shopping with Nutresa! | info@nutresa.com | Vijayawada, Andhra Pradesh',
