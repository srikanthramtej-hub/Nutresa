...
let MailService = class MailService {
    ...
    async sendOrderConfirmation(to, order, gst) {
      ...
      <td style="padding:8px;border-bottom:1px solid #f0e6d3;text-align:right">₹${(i.price * i.qty).toFixed(2)}</td>
        ...
        const pdfBuffer = await (0, pdf_util_1.generateOrderPDF)(order, gst);
    ...
    async sendAdminOrderNotification(order, gst) {
    ...
    async sendReorderReminder(order) {
